...
// emailVerifiedHandler writes the User's EmailVerified condition from Zitadel's email
...
// failures, because the condition is the state milo's controllers gate on: a silent
...
func (s *Server) emailVerifiedHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Error(err, "Failed to write EmailVerified condition", "userId", userID)
		http.Error(w, "failed to update user", http.StatusInternalServerError)
		return
	}

	log.Info("Recorded EmailVerified condition",
