...
var (
	...
	sweepEmailVerifiedUpdates = prometheus.NewCounter(prometheus.CounterOpts{
		...
		Help: "EmailVerified conditions written by the sweeper because they disagreed with Zitadel.",
	})
)
	...
	for {
		...
		for i := range users {
			...
			if existingUser, ok := existing[u.ID]; ok {
				// C11: reconcile the EmailVerified condition. This is the backfill for
				...
				if emailverified.NeedsUpdate(existingUser, u.IsEmailVerified) {
					...
					case err != nil:
						...
						sweepErrors.Inc()
						log.Error(err, "Failed to reconcile EmailVerified condition", "zitadelUserId", u.ID)
					case changed:
