...
var _ = ginkgo.Describe("UserSweeper", func() {
	...
	ginkgo.It("diffs against a single List instead of per-user Gets", func() {
		...
		// steady state after the first backfill: the EmailVerified reconcile must
	...
	ginkgo.Context("EmailVerified reconcile", func() {
		...
		userWith := func(name string, conds ...metav1.Condition) *iammiloapiscomv1alpha1.User {
			return &iammiloapiscomv1alpha1.User{
				ObjectMeta: metav1.ObjectMeta{Name: name},
				Status:     iammiloapiscomv1alpha1.UserStatus{Conditions: conds},
			}
		}

		nowCond := func(c metav1.Condition) metav1.Condition {
			c.LastTransitionTime = metav1.Now()
			return c
		}

		ginkgo.It("writes the condition when it disagrees with Zitadel", func() {
			before := testutil.ToFloat64(sweepEmailVerifiedUpdates)
			k8sFake := fake.NewClientBuilder().WithScheme(scheme).
				WithStatusSubresource(&iammiloapiscomv1alpha1.User{}).
				WithObjects(userWith("u-stale", nowCond(emailverified.Desired(false)))).
				Build()
			lister := &mockUserLister{pages: [][]zitadel.User{{
			...
			gomega.Expect(emailverified.IsTrue(&got)).To(gomega.BeTrue())
			gomega.Expect(testutil.ToFloat64(sweepEmailVerifiedUpdates) - before).To(gomega.Equal(1.0))
		})

		ginkgo.It("issues no write when the condition already agrees", func() {
			statusUpdates, getCalls := 0, 0
			k8sFake := fake.NewClientBuilder().WithScheme(scheme).
				WithStatusSubresource(&iammiloapiscomv1alpha1.User{}).
				WithObjects(userWith("u-agrees", nowCond(emailverified.Desired(true)))).
				WithInterceptorFuncs(interceptor.Funcs{
					Get: func(ctx context.Context, c client.WithWatch, key client.ObjectKey, obj client.Object, opts ...client.GetOption) error {
						getCalls++
						return c.Get(ctx, key, obj, opts...)
		...
		ginkgo.It("backfills a user that has no condition at all", func() {
			k8sFake := fake.NewClientBuilder().WithScheme(scheme).
				WithStatusSubresource(&iammiloapiscomv1alpha1.User{}).
		...
		ginkgo.It("does not abort the sweep when one condition write fails", func() {
			k8sFake := fake.NewClientBuilder().WithScheme(scheme).
				WithStatusSubresource(&iammiloapiscomv1alpha1.User{}).
				WithObjects(
					userWith("u-a", nowCond(emailverified.Desired(false))),
					userWith("u-b", nowCond(emailverified.Desired(false))),
				).
				WithInterceptorFuncs(interceptor.Funcs{
					SubResourceUpdate: func(ctx context.Context, c client.Client, sub string, obj client.Object, opts ...client.SubResourceUpdateOption) error {
						if obj.GetName() == "u-a" {
